...
int main(void)
{
    ...
    /* The live shadow producer gate is exact: both proven local zetas with a
     * depth-only writing draw are refused, while a color-writing pass, a
     * read-only depth pass, main-memory aliases and unrelated zetas remain
     * eligible for complete-section native admission. */
    CHECK(rsx_nr_yz_unproven_shadow_depth_producer(
              0u, RSX_NR_YZ_SHADOW_ZETA0, 0u, 1u));
    CHECK(rsx_nr_yz_unproven_shadow_depth_producer(
              0u, RSX_NR_YZ_SHADOW_ZETA1, 0u, 1u));
    CHECK(!rsx_nr_yz_unproven_shadow_depth_producer(
               0u, RSX_NR_YZ_SHADOW_ZETA0, 0x01010101u, 1u));
    CHECK(!rsx_nr_yz_unproven_shadow_depth_producer(
               0u, RSX_NR_YZ_SHADOW_ZETA0, 0u, 0u));
    CHECK(!rsx_nr_yz_unproven_shadow_depth_producer(
               1u, RSX_NR_YZ_SHADOW_ZETA0, 0u, 1u));
    CHECK(!rsx_nr_yz_unproven_shadow_depth_producer(
