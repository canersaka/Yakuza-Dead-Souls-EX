...
I got the entire graphics command pipeline standing up and behaving exactly like a real PS3 (real rendering comes later, but the plumbing works). I added support for actually running the PS3's SPU cores. Instead of faking the PS3's multi-core job system (SPURS), it's been recompiled and now executes for real, with the core scheduler running as native SPU code on its own threads.

Since then I've pushed it a good bit further. It's now past SPURS entirely and runs through the system, trophy and message-dialog screens, and it reads its own data files straight off the disc (a couple of endianness and struct-packing bugs were hiding in the filesystem layer, and the dialog handler was calling a guest callback the wrong way). The big one that had been stalling the boot for a while turned out to be a recompiler bug, not a game bug: a jump-table switch the lifter had quietly stubbed out, which I tracked down by teaching the crash handler to walk the guest call stack. With that fixed it now runs its actual per-frame render loop, generating real RSX graphics commands every frame. The next big step is the part that consumes them, hooking up the GPU backend and opening a real window. Full rendering is the long road after that.
