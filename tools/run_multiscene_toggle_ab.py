#!/usr/bin/env python3
"""Interleaved, retry-bounded comparison for one production build toggle."""
...
def main():
    ...
    for lane_name, repeat in plan:
        ...
        for attempt in range(1, args.attempts + 1):
            tag = (
                f"multiscene-{args.experiment}-{lane_name}-r{repeat}-"
            ...
            print(
                f"[multiscene-{args.experiment}] {lane_name} "
                f"repeat={repeat} attempt={attempt}", flush=True
            )
            run = subprocess.run(command, cwd=worktree)
            result = None
            if result_path.is_file():
                result = json.loads(result_path.read_text(encoding="utf-8"))
            if run.returncode == 0 and result and result.get("status") == "passed":
                invalid = [
    ...
    comparison = {}
    for scene_name in completed["off"][0]["scenes"]:
        off = [run["scenes"][scene_name]["fps_mean"] for run in completed["off"]]
        on = [run["scenes"][scene_name]["fps_mean"] for run in completed["on"]]
        off_mean, on_mean = mean(off), mean(on)
    ...
    output_dir = scratch / f"multiscene-{args.experiment}-ab-{args.date}"
    ...
    print(f"[multiscene-{args.experiment}] summary: {summary_path}", flush=True)
    return 0
