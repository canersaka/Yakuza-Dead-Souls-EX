#!/usr/bin/env python3
"""Run an interleaved, retry-bounded scalar/SIMD LS128 scene comparison."""
...
def main():
    ...
    lanes = {
        "scalar": {
            "toggle": "OFF",
            "exe": worktree / "yakuza" /
                   f"lane_{experiment}_scalar_20260821" /
                   "yakuza_recomp.exe",
        },
        "simd": {
            "toggle": "ON",
            "exe": worktree / "yakuza" /
                   f"lane_{experiment}_simd_20260821" /
                   "yakuza_recomp.exe",
        },
    }
    ...
    for lane_name, repeat in plan:
        ...
        for attempt in range(1, MAX_ATTEMPTS + 1):
            tag = (
                f"multiscene-{experiment}-{lane_name}-r{repeat}-"
            ...
            print(
                f"[multiscene-{experiment}] {lane_name} repeat={repeat} "
    ...
    scalar_toggle = scalar_common.pop(toggle_key)
    simd_toggle = simd_common.pop(toggle_key)
    if scalar_common != simd_common or scalar_toggle != "OFF" or (
            simd_toggle != "ON"):
        raise AssertionError("lane configuration differs beyond LS128 toggle")

    output_dir = scratch / f"multiscene-{experiment}-ab-20260821"
    ...
    print(f"[multiscene-{experiment}] summary: {summary_path}", flush=True)
    return 0
