...
CLI:
    py -3 tools\\lift_audit.py [--baseline scratch\\lift_audit_baseline.txt]
                                [--update-baseline]
...
def marker_census():
    """Scan recomp/ppu_recomp_*.cpp for the two marker emit sites.
    ...
    """
    chunk_paths = sorted(glob.glob(os.path.join(ROOT, "recomp", "ppu_recomp_*.cpp")))
    if not chunk_paths:
        print("ERROR: no recomp/ppu_recomp_*.cpp chunk files found -- "
              "has the project ever been relifted? (recomp/ is generated, "
              "never hand-edited; run tools/ppu_lifter.py first)",
              file=sys.stderr)
        sys.exit(2)
...
def fallthrough_census(elf_data: bytes):
    entries = _load_functions_json()
...
def main():
    ...
    elf_path = os.path.join(ROOT, "game", "EBOOT.elf")
    ...
    chunk_paths, todo_by_mnemonic, unresolved_addrs = marker_census()
    fn_entries, gap_followed, ft_hazards = fallthrough_census(elf_data)
    total_words, total_targets, gap_targets, midfunc_targets = \
