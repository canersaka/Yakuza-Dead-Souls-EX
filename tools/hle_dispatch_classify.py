...
invoked at runtime. Our runtime has three dispatch layers per PS3 import
NID, all resolved statically in yakuza/import_bridges_gen.cpp's
`yz_imports[]` table (each entry: `{ NID, fn, lle_opd }`, with a
`/* module::name */` trailing comment naming the PS3 export):
  ...
  2. fn is a `yz_ovr_<Name>` symbol (defined in yakuza/import_overrides.cpp)
  ...
  3. fn is `yz_imp_<Name>` (the generated marshaling wrapper)
         -> HLE-DISPATCHED: `gen_imports.py`'s mechanical convention --
            confirmed by reading yakuza/import_bridges_gen.cpp -- is that
            `yz_imp_<Name>` ALWAYS calls the libs/ C function named
            `<Name>` directly (e.g. yz_imp_cellFsClose calls cellFsClose()
            from libs/filesystem/cellFs.c). This is the one case where a
            libs/ signature mismatch is a real, live ABI bug.
  4. fn is `yz_imp_stub_<module>_<hex>`
  ...
  (not found in yz_imports[] at all)
         -> UNKNOWN: this PS3 export is not in Yakuza's import table, i.e.
            not imported by this game build -- the mismatch, live or not,
            can never fire.

Ground-truth sanity check (per the task): cellGcmGetTiledPitchSize MUST
classify as SHADOWED (LLE) -- see import_bridges_gen.cpp:1864,
...
# yz_ovr_<Name> function bodies in import_overrides.cpp, so we can test
# whether an override delegates to the libs/ function of the same name.
_OVR_DEF_RE = re.compile(
    r"^\s*(?:extern\s+\"C\"\s+)?(?:static\s+)?\w[\w\s\*]*?\b(?P<sym>yz_ovr_" + _IDENT + r")\s*\(\s*ppu_context\s*\*[^)]*\)\s*\{",
    re.MULTILINE,
)
...
def _classify_row(fn: str, lle_opd: str) -> str:
...
def scan_override_bodies(overrides_path: str) -> dict:
    """Returns yz_ovr_<Name> -> True/False: does the override body call the
    libs/ C function `<Name>(...)` (same bare name, i.e. delegates to the
    HLE impl the audit compared) or not (fully self-contained / calls a
    DIFFERENT libs/ function / calls none)?

    Method: slice each yz_ovr_<Name> function body (brace-matched) and
    search for a call to the identifier `<Name>` immediately followed by
    '(' -- i.e. `Name(...)`, not `yz_ovr_Name(...)` or some other symbol.
    This is a textual heuristic, not a semantic one -- labeled as such in
    the report.
    """
    ...
    result = {}
    for m in _OVR_DEF_RE.finditer(text_nc):
        sym = m.group("sym")
        bare_name = sym[len("yz_ovr_"):]
...
def build_classified_report(libs_root: str, oracle_root: str, whitelist_path: str,
                             bridges_path: str, overrides_path: str) -> dict:
    base = audit.build_report(libs_root, oracle_root, whitelist_path)
    dispatch = parse_bridge_table(bridges_path)
    override_delegates = scan_override_bodies(overrides_path)
...
def main(argv=None) -> int:
    ...
    rep = build_classified_report(args.libs, args.oracle, args.whitelist, args.bridges, args.overrides)
