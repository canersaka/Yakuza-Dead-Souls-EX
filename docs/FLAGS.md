...
| `YZ_RSX_DRAW` | libs/video/rsx_live_draw.c (`rsx_live_draw_enabled`) | **Kill-switch for the Track B live NV4097 draw path (2026-07-03, Track B stage 5 B2).** Default ON; `=0` makes the live-draw engine inert so the runtime keeps its existing (null/clear-color) present. The engine is the validated capture-replay D3D12 pipeline (rsx_dispatch + NV40 VP/FP decompilers + PSO/sampler/mip engine) driven by the live FIFO consumer. **NOT YET WIRED into yakuza/import_overrides.cpp** — module compiles + links standalone; the hook (`rsx_live_draw_method`/`_present`) is pending the parent's integration + a live boot (the game still stalls at the CRI movie gate, so live draws can't be validated end-to-end yet). See scratch/trackb_stage5_report.md. |
